...
class PartenaireController extends AbstractController
{
    ...
    public function new(Request $request): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $entityManager->flush();
            $this->addFlash("success", "Enregistrement du partenaire " . $partenaire->getDesignation() . " réussi  ");
    ...
    public function edit(Request $request, Partenaire $partenaire): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $this->addFlash("warning", "Modification du partenaire " . $partenaire->getDesignation() . " réussi  ");
            return $this->redirectToRoute('partenaire_index');
        }
    ...
    public function delete(Request $request, Partenaire $partenaire): Response
    {
        if ($this->isCsrfTokenValid('delete' . $partenaire->getId(), $request->request->get('_token'))) {
            $entityManager = $this->getDoctrine()->getManager();
            $nom = $partenaire->getDesignation();

            $entityManager->remove($partenaire);
            $entityManager->flush();
            $this->addFlash("danger", "Supression du partenaire " . $nom . " réussi  ");
        }
